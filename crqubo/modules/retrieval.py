...
class RetrievalModule:
    ...
    def retrieve(
        ...
        **kwargs
        ...
        if min_relevance > 0:
            documents = [doc for doc in documents if doc.relevance_score >= min_relevance]
...
class HybridRetriever(BaseRetriever):
    ...
    def retrieve(self, query: str, top_k: int = 5, **kwargs) -> List[RetrievedDocument]:
        ...
        semantic_results = self.semantic_retriever.retrieve(query, top_k=top_k, **kwargs)
        
        # Get keyword results if available
        keyword_results = []
        if self.keyword_retriever:
            keyword_results = self.keyword_retriever.retrieve(query, top_k=top_k, **kwargs)
    ...
    def _merge_results(
        ...
        for doc in keyword_results:
            ...
            if doc_id in doc_map:
                # Combine scores
                doc_map[doc_id].relevance_score += doc.relevance_score * (1 - self.alpha)
            else:
                doc_map[doc_id] = RetrievedDocument(
                    content=doc.content,
                    metadata=doc.metadata,
        ...
        sorted_results = sorted(doc_map.values(), key=lambda x: x.relevance_score, reverse=True)
