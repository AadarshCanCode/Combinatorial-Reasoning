...
class Z3Verifier(BaseVerifier):
    ...
    def verify_reasons(
        ...
        **kwargs,
        ...
        is_consistent = (
            ...
            and contradiction_result["is_consistent"]
            and circularity_result["is_consistent"]
        )
    ...
    def _parse_reasoning_steps(
        self, reasoning_steps: List[str], verification_level: VerificationLevel
    ) -> List[Dict[str, Any]]:
        """Parse reasoning steps into logical formulas."""
        formulas = []
    ...
    def _extract_constraints(self, step: str) -> List[Dict[str, Any]]:
        ...
        causal_matches = re.findall(
            r"(\w+)\s+(causes?|leads to|results in)\s+(\w+)", step, re.IGNORECASE
        )
    ...
    def _classify_reasoning_type(self, step: str) -> str:
        ...
        if any(word in step_lower for word in ["if", "then", "implies", "therefore"]):
            ...
            word in step_lower
            for word in ["causes", "leads to", "results in", "because"]
        ):
            return "causal"
        elif any(
            word in step_lower
            for word in ["equals", "=", "greater", "less", "more", "fewer"]
        ):
            return "quantitative"
        elif any(
            word in step_lower
            for word in ["before", "after", "during", "when", "while"]
        ):
            return "temporal"
        else:
            return "general"
    ...
    def _check_logical_consistency(
        self, formulas: List[Dict[str, Any]], verification_level: VerificationLevel
    ) -> Dict[str, Any]:
        ...
        if result == z3.sat:
            ...
            return {
                ...
                "score": 0.0,
                "issues": [
                    "Logical inconsistency detected: formulas are unsatisfiable"
                ],
            }
        else:
            return {
    ...
    def _check_circular_reasoning(self, reasoning_steps: List[str]) -> Dict[str, Any]:
    ...
    def _are_contradictory(
        self, formula1: Dict[str, Any], formula2: Dict[str, Any]
    ) -> bool:
    ...
    def _constraints_contradict(
        ...
        if (
            constraint1["type"] == "equality"
            and constraint2["type"] == "inequality"
            and constraint1["left"] == constraint2["left"]
            and constraint1["right"] == constraint2["right"]
        ):
            return True

        # Check for opposite inequalities
        if (
            constraint1["type"] == "inequality"
            and constraint2["type"] == "inequality"
            and constraint1["left"] == constraint2["left"]
            and constraint1["right"] == constraint2["right"]
        ):
    ...
    def _is_circular_reference(
        self, step: str, prev_step: str, step_idx: int, prev_idx: int
    ) -> bool:
    ...
    def verify_reasons(
        ...
        **kwargs,
        ...
        result = self.verifier.verify_reasons(
            reasoning_steps=reasoning_steps, query=query, **verify_params
        )

        return result.verified_steps

    def get_verification_stats(
        self, original_steps: List[str], verified_steps: List[str]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "verification_rate": (
                len(verified_steps) / len(original_steps) if original_steps else 0
            ),
